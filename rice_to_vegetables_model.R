...
transition_rice_to_vegetables <- function(x, varnames){
  ...
  rice_total_benefit_precal <- sum(rice_income, rice_water_use, rice_nutrition)
  ...
  vegetables_income_precal <- sum(potato_income, shallot_income, cabbage_income)
  ...
  vegetables_total_benefit_precal <- sum(vegetables_income, vegetables_water_use, vegetables_nutrition)
  ...
  crop_rotation_income1_precal <- sum(potato_income, rice_income, shallot_income)
  crop_rotation_income1 <- vv(crop_rotation_income1_precal, n_year, var_CV=CV_value)
  crop_rotation1_total_benefits_precal <- sum(crop_rotation_income1,
  ...
  crop_rotation2_total_benefits_precal <- sum(crop_rotation_income2,
                                              crop_rotation_water_use,
                                              crop_rotation_nutrition)
  ...
  crop_rotation3_total_benefits_precal <- sum(crop_rotation_income3,
                                              crop_rotation_water_use,
                                              crop_rotation_nutrition)
  ...
  # Estimate the cost
  ##if farmers grow rice (assuming they grow rice 3 times per year)
  rice_costs_precal <- sum(rice_farming_input_costs, rice_labor_costs,rice_machinery_costs)*3 +
  ...
  ##if farmers grow vegetables 
  ##(assuming they grow 3 different vegetables per year)
  potato_costs_precal <- sum(potato_farming_input_costs, potato_labor_costs, potato_machinery_costs)
  potato_costs <- vv(potato_costs_precal, n_year, var_CV=CV_value)
  
  shallot_costs_precal <- sum(shallot_farming_input_costs, shallot_labor_costs, shallot_machinery_costs)
  shallot_costs <- vv(shallot_costs_precal, n_year, var_CV=CV_value)
  
  cabbage_costs_precal <- sum(cabbage_farming_input_costs, cabbage_labor_costs, cabbage_machinery_costs)
  cabbage_costs <- vv(cabbage_costs_precal, n_year, var_CV=CV_value)
  
  vegetables_costs_precal <- sum(potato_costs, shallot_costs, cabbage_costs, 
  ...
  crop_rotation_cost1_precal <- sum(potato_costs, rice_total_costs, shallot_costs)
  crop_rotation_cost1 <- vv(crop_rotation_cost1_precal, n_year, var_CV=CV_value)
  crop_rotation1_total_costs_precal <- sum(crop_rotation_cost1, 
                                            vegetables_farming_equipment_costs,
                                            warehouse_establishment_costs, 
                                            warehouse_maintenance_costs)
  ...
  crop_rotation_cost2_precal <- sum(potato_costs, rice_total_costs, cabbage_costs)
  crop_rotation_cost2 <- vv(crop_rotation_cost2_precal, n_year, var_CV=CV_value)
  crop_rotation2_total_costs_precal <- sum(crop_rotation_cost2, 
                                           vegetables_farming_equipment_costs,
                                           warehouse_establishment_costs, 
                                           warehouse_maintenance_costs)
  ...
  crop_rotation_cost3_precal <- sum(shallot_costs, rice_total_costs, cabbage_costs)
  crop_rotation_cost3 <- vv(crop_rotation_cost3_precal, n_year, var_CV=CV_value)
  crop_rotation3_total_costs_precal <- sum(crop_rotation_cost3, 
                                           vegetables_farming_equipment_costs,
                                           warehouse_establishment_costs, 
                                           warehouse_maintenance_costs)
